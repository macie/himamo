...
class RecomputeEmissionsTestCase(BaseTestCase):
    def smoke_test_recompute_emissions(self):
        # N = 2, T = 3
        log_gamma = np.array([
        ...
        symbols = np.array(
            [d(1), d(2), d(3)])
        result = self.model._recompute_emissions(log_gamma,
                                                 observed_states,
                                                 symbols)
        expected_result = np.array([
            [d(1)/d(6), d(5)/d(6), d(0)],
            [d(4)/d(15), d(11)/d(15), d(0)]])
    ...
    def test_empty_loggamma(self):
        ...
        with self.assertRaises(ValueError):
            self.model._recompute_emissions(log_gamma,
                                            observed_states,
                                            symbols)
    ...
    def test_numerical_stability_if_increased_time(self):
        for i in xrange(0, 3):
            ...
            result = self.model._recompute_emissions(log_gamma,
                                                     observed_states,
                                                     symbols)
            expected_result = np.array([[d(1)]*(8**i)]*2)
            np.testing.assert_almost_equal(
                result, expected_result, decimal=self.num_precision-1)
    ...
    def test_numerical_stability_if_increased_number_of_states(self):
        for i in xrange(0, 5):
            ...
            result = self.model._recompute_emissions(log_gamma,
                                                     observed_states,
                                                     symbols)
            expected_result = np.array([[d(1)]*2]*(8**i))
            np.testing.assert_array_equal(result, expected_result)
