...
class RecomputeLogInitialStatesTestCase(BaseTestCase):
    def smoke_test_recompute_log_initial_states(self):
        log_gamma = np.array([
            [d(1).ln(), d(2).ln()],
            [d(1).ln(), d(2).ln()],
            [d(1).ln(), d(2).ln()]])
    ...
    def test_empty_loggamma(self):
        ...
        with self.assertRaises(ValueError):
            self.model._recompute_log_initial_states(log_gamma)
    ...
    def test_numerical_stability_if_increased_time(self):
        for i in xrange(0, 8):
            ...
            result = self.model._recompute_log_initial_states(log_gamma)
            expected_result = np.array([d(1).ln()]*2)
            np.testing.assert_array_equal(result, expected_result)
    ...
    def test_numerical_stability_if_increased_number_of_states(self):
        for i in xrange(0, 7):
            ...
            result = self.model._recompute_log_initial_states(log_gamma)
            expected_result = np.array([d(1).ln()]*(8**i))
            np.testing.assert_array_equal(result, expected_result)
