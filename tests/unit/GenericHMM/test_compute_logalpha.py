...
class ComputeLogAlphaTestCase(BaseTestCase):
    ...
    def smoke_test_compute_logalpha(self):
        ...
        result = self.model._compute_logalpha(pi, a, b)
        expected_result = np.array([
            [d(1).ln(), d(2).ln(), d(4).ln()],
            [d(1).ln(), d(2).ln(), d(4).ln()]])
        np.testing.assert_array_equal(result, expected_result)

        pi, a, b = self._testing_parameters_generator(3, 2, d(1), d(2), d(2))
        result = self.model._compute_logalpha(pi, a, b)
        expected_result = np.array([
            [d(2).ln(), d(24).ln()],
            [d(2).ln(), d(24).ln()],
    ...
    def test_empty_initial_states_matrix(self):
        ...
        with self.assertRaises(ValueError):
            self.model._compute_logalpha(pi, a, b)
